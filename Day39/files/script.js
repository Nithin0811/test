...
async function loaddata() {
    let dataraw = await fetch("http://localhost:8000/api/mentor");
...
async function addmentor() {
    ...
    await fetch('http://localhost:8000/api/mentor', {
        method: "POST",
...
async function deletementor(mentorid) {
    try {
        let res = await fetch(`http://localhost:8000/api/mentor/${mentorid}`, {
            method: "DELETE"
                // headers: {
                //     "Content-type": "application/json"
                // }
...
async function getstudent(mentorname) {
    try {
        let rawdata = await fetch(`http://localhost:8000/api/student/${mentorname}`);
